...
from core.observability.prometheus import (APP_NAME, EXCEPTIONS_TOTAL,
                                           IN_PROGRESS, REQUEST_DURATION,
                                           REQUEST_ERRORS,
                                           REQUEST_STATUS_CLASS, REQUEST_TOTAL)
        ...
        async def send_wrapper(message: Send):
            if message["type"] == "http.response.start":
                ...
                REQUEST_TOTAL.labels(
                    app=self.app_label, method=method,
                    endpoint=endpoint, status=status_str
                ).inc()
                ...
                REQUEST_STATUS_CLASS.labels(
                    app=self.app_label, method=method,
                    endpoint=endpoint, status_class=f"{status_code // 100}xx"
                ).inc()
