...
async def get_regions_overview(
    request: Request, service: RegionAnalyticsService = Depends(get_region_analytics_service),
):
    return await run_analytics_operation(request, "analytics.regions.overview", {}, service.get_overview, lambda: [])


@router.get("/{region_id}/details", response_model=RegionDetailsOut | None, dependencies=[Depends(AssetPermissions.CanViewAssets)])
@cached(ttl=180, tags=("analytics:regions:details",))
async def get_region_details(
    request: Request, region_id: UUID,
    service: RegionAnalyticsService = Depends(get_region_analytics_service),
):
    return await run_analytics_operation(
        request, "analytics.regions.details", {"region_id": region_id},
        lambda: service.get_details(region_id),
        lambda: None,
    )


@router.get("/heatmap", response_model=list[RegionHeatmapPointOut], dependencies=[Depends(AssetPermissions.CanViewAssets)])
@cached(ttl=300, tags=("analytics:regions:heatmap",))
async def get_region_heatmap(
    request: Request, service: RegionAnalyticsService = Depends(get_region_analytics_service),
):
    return await run_analytics_operation(request, "analytics.regions.heatmap", {}, service.get_heatmap, lambda: [])
