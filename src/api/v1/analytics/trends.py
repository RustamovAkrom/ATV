...
@router.get("/assignments", response_model=TrendSeriesOut, dependencies=[presets.CanViewAssets])
@cached(ttl=300, tags=("analytics:trends:assignments",))
async def get_assignment_trends(
    request: Request,
    interval: TrendInterval = Query(default=TrendInterval.DAILY),
    periods: int = Query(default=30, ge=1, le=180),
...
@router.get("/transfers", response_model=TrendSeriesOut, dependencies=[presets.CanViewAssets])
...
@router.get("/repairs", response_model=RepairTrendSeriesOut, dependencies=[presets.CanViewAssets])
