...
async def get_dashboard_overview(
    request: Request,
    assignment_service: AssetAssignmentAnalyticsService = Depends(get_asset_assignment_analytics_service),
    transfer_service: AssetTransferAnalyticsService = Depends(get_asset_transfer_analytics_service),
    history_service: AssetHistoryAnalyticsService = Depends(get_asset_history_analytics_service),
):
    ...
    return await run_analytics_operation(
        ...
        lambda: {
            "period": "all_time",
            "assignment_metrics": {"total_active": 0, "total_inactive": 0, "total_assignments": 0},
