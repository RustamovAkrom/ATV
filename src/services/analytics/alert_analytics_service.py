        ...
        for row in overloaded_users:
            alerts.append(
                AlertOut.model_construct(
                    ...
                    severity=(
                        AlertSeverity.CRITICAL
                        if int(row.active_assignments or 0)
                        > assignment_threshold
                        * settings.ALERT_REPAIR_THRESHOLD_MULTIPLIER
                        else AlertSeverity.WARNING
                    ),
                    entity_id=row.id,
