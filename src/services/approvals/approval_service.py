# src/services/approvals/approval_service.py
import builtins
...
from db.models.enums import ApprovalStatus, UserStatus
...
class ApprovalService:
    def __init__(
        ...
        # Регистрируем все обработчики и внедряем зависимости
        self._init_handlers()
    ...
    def _init_handlers(self):
        ...
        for handler in ApprovalHandlerRegistry.get_all().values():
            if hasattr(handler, "set_services"):
                handler.set_services(
                    asset_service=self.asset_service,
                    transfer_service=self.transfer_service,
                    repair_service=self.repair_service,
                    asset_assignment_service=self.asset_assignment_service,
    ...
    async def create(
        ...
        # Валидация через хендлер
        validated_payload = await self._validate_request_v2(data)
    ...
    async def _validate_request_v2(self, data: ApprovalCreate) -> dict:
        """Новая версия валидации через хендлеры"""
        entity_type = data.entity_type.strip().lower()
        action = data.action.strip().lower()

        handler = ApprovalHandlerRegistry.get(entity_type, action)

        if not handler:
            supported = ApprovalHandlerRegistry.get_supported_keys()
            raise BadRequest(
                f"Unsupported approval request: ({entity_type}, {action}). "
    ...
    async def approve(
        ...
        async with self.approval_repo.session.begin_nested():
            ...
            await self._execute_approved_action_v2(approval, actor)
    ...
    async def _execute_approved_action_v2(
        self, approval: ApprovalRequest, actor: CurrentUserSchema
    ) -> None:
        """Новая версия выполнения через хендлеры"""
        handler = ApprovalHandlerRegistry.get(approval.entity_type, approval.action)

        if not handler:
            raise BadRequest(
                f"No handler found for {approval.entity_type}:{approval.action}"
            )
