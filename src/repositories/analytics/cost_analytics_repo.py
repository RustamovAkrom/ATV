...
class CostAnalyticsRepository(BaseAnalyticsRepository):
    ...
    async def list_repair_costs(self, pagination):
        ...
        query = select(
            ...
            (func.coalesce(Repair.labor_cost, 0) + func.coalesce(repair_parts.c.parts_cost, 0)).label("total_cost"),
        ).join(Asset, Asset.id == Repair.asset_id).outerjoin(
            repair_parts, repair_parts.c.repair_id == Repair.id
        ).order_by(
            (func.coalesce(Repair.labor_cost, 0) + func.coalesce(repair_parts.c.parts_cost, 0)).desc(),
    ...
    async def list_region_costs(self, pagination):
        ...
        ).outerjoin(purchase_costs, purchase_costs.c.region_id == Region.id).outerjoin(
            repair_costs, repair_costs.c.region_id == Region.id
        ).order_by(
