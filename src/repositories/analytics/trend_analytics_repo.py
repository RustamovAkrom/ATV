...
class TrendAnalyticsRepository(BaseAnalyticsRepository):
    """Репозиторий для аналитики трендов"""
    ...
    async def assignment_counts(
        ...
        """Количество назначений по периодам"""
        ...
        query = select(
            ...
            AssetAssignment.assigned_at >= start,
            AssetAssignment.assigned_at <= end
    ...
    async def transfer_counts(
        ...
        """Количество трансферов по периодам"""
        ...
        query = select(
            ...
            AssetTransfer.created_at >= start,
            AssetTransfer.created_at <= end
        ).group_by(bucket).order_by(bucket.asc())
    ...
    async def repair_counts(
        ...
        """Количество и стоимость ремонтов по периодам"""
        repair_parts = (
            select(
        ...
        ).outerjoin(repair_parts, repair_parts.c.repair_id == Repair.id).where(
            Repair.created_at >= start,
            Repair.created_at <= end
        ).group_by(bucket).order_by(bucket.asc())
