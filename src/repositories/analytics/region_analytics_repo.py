...
class RegionAnalyticsRepository:
    ...
    async def get_details(self, region_id):
        ...
        service_rows = await self.session.execute(
            ...
            .outerjoin(
                AssetAssignment,
                (AssetAssignment.asset_id == Asset.id) & AssetAssignment.unassigned_at.is_(None),
            )
        ...
        cost_rows = await self.session.execute(
            ...
            .group_by(Asset.id, Asset.name, Asset.asset_tag, Asset.purchase_cost, repair_costs.c.repair_cost)
            .order_by(
