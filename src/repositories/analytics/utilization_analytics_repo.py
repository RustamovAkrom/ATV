...
class UtilizationAnalyticsRepository(BaseAnalyticsRepository):
    ...
    async def metrics(
        ...
        # Загрузка регионов
        region_load_query = select(
            Region.id.label("region_id"),
            Region.name.label("region_name"),
            func.count(Asset.id).label("asset_count"),
            func.count(func.distinct(Asset.owner_id)).label("employee_count"),
        ).outerjoin(Asset, Asset.region_id == Region.id).where(
            *filters
        ...
        avg_load = sum(r["load_ratio"] for r in region_load_list) / len(region_load_list) if region_load_list else 0
        overloaded = [r for r in region_load_list if r["load_ratio"] > avg_load * 1.2]
        underutilized = [r for r in region_load_list if r["load_ratio"] < avg_load * 0.5]
