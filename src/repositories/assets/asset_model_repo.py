...
class AssetModelRepository(BaseRepository):
    ...
    async def list(self):
        return await self.scalars(select(AssetModel).order_by(AssetModel.name))

    async def get(self, model_id):
        return await self.scalar(select(AssetModel).where(AssetModel.id == model_id))

    async def get_by_slug(self, slug: str):
        return await self.scalar(select(AssetModel).where(AssetModel.slug == slug))
